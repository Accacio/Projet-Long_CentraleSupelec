...
%%  ----- Exemple de simulation pour la commande du PID angulaire
%
% th_DHM = % définir la position angulaire initial dans la configuration DHM 
% trajectory = % définir une trajectoire angulaire. Si on utilise juste une consigne en échelon, changer la clé dans le schéma e mettre 0 comme valeur ici.
...
%%   ----- Exemple de simulation pour la commande cascade
%
% th_DHM = % définir la position angulaire initial dans la configuration DHM 
%
% % Calcul de la trajectoire pour l'outil du robot
% initP = MGD_end(d1,d2,d3,d4,lend,r1,th_DHM(1),th_DHM(2),th_DHM(3),th_DHM(4));
% finalP = [0.3970; 0.0500; 0.3650];
% p3 = [0.4470; 0.0000; 0.365];
% type = 'l';
% T = 5;
% t = 0.001;
% trajectory = traj(initP,finalP,p3,type,T,t,0);
%
% % Estimation numérique de la vitesse de l'outil
...
% 
% % Réalisation du graphic 3D de comparaison de la trajectoire calculée de la vraie trajectoire du robot
% figure;
...
%%   ----- Exemple de simulation pour la commande par MGI
% 
% th_DHM = % définir la position angulaire initial dans la configuration DHM 
%
% % Calcul de la trajectoire pour l'outil du robot
...
% T = 10;
% t = 0.001;
% lin_trajectory = traj(initP,p3,finalP,type,T,t,0);
%
% % Détermination de la trajectoire initial pour l'orientation de l'outil
% trajectory = zeros(5,size(lin_trajectory,2)*2);
% init_angle = (inverse_geometric_model((lin_trajectory(:,1))'))';
% for i = 1:size(lin_trajectory,2)
%     trajectory(:,i) = init_angle + (th_DHM - init_angle)*exp(-i/(2*1000));
% end
%
% % Conversion de la trajectoire lineaire désirée à l'espace articulaire
% for i = (size(lin_trajectory,2)+1):(2*size(lin_trajectory,2))
%     j = i - size(lin_trajectory,2);
%     pos = lin_trajectory(:,j)';
%     trajectory(:,i) = (inverse_geometric_model(pos))';
% end
% 
% sim('dynmodel_KUKA_PIDjoint_2014')
% 
% % Réalisation du graphic 3D de comparaison de la trajectoire calculée de la vraie trajectoire du robot
% figure;
% plot3(pos_end.signals.values(:,1),pos_end.signals.values(:2,2),pos_end.signals.values(:,3))
% hold on
% plot3(lin_trajectory(1,:),lin_trajectory(2,:),lin_trajectory(3,:),'r')
% hold off
% axis equal